...
# install app dependencies(add cargo)
RUN apt-get install -y libboost-dev libboost-filesystem-dev libboost-serialization-dev \
    ...
    libluajit-5.1-dev libssl-dev libfstrm-dev cargo

# download source code
RUN wget https://downloads.powerdns.com/releases/pdns-recursor-4.9.0.tar.bz2

# extract the source code
RUN tar -xf pdns-recursor-4.9.0.tar.bz2

# build from source code
RUN cd pdns-recursor-4.9.0 && ./configure --sysconfdir=/etc/powerdns/ --enable-dnstap && make -j4 && make install
